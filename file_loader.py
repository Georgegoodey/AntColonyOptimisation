...
from tkinter_objects import FileObject
...
class Loader:
    ...
    def loadFile(self, filepath:str, fileInfo:FileObject) -> None:
        filetype = os.path.splitext(filepath)[1][1:]
        self.fileInfo = fileInfo
        self.coords = []
        if(filetype == "csv"):
            self.coords = self.loadCSV(filename=filepath)
        elif(filetype == "tsp"):
            self.coords = self.loadTSP(filename=filepath)
        elif(filetype == "txt"):
            self.coords = self.loadTXT(filename=filepath)
    ...
    def loadTSP(self, filename:str):
        ...
        if(lines[1].startswith("COMMENT")):

            comment = lines[1].split(":")[1][1:-1]

        self.fileInfo.editVars(fileName=str(name), fileComment=str(comment), nodes=dimension, sol=(tour!=None))
